...
# ============================================================
# CONFIG — Railway: змінні середовища, локально: вставте значення
# ============================================================
...
SIGNALS_DUMP   = "signals_dump.json"   # <-- новий файл з повними даними для аналізу
...
def detect_amd(c15, c5m, price):
    ...
    if manip_ok:
        if sweep["type"] == "LOW":
            d = "UP"; reason = "SWEEP_LOW: potential reversal UP"
        else:
            last_close = c15[-1]["c"]
            if last_close < sweep["level"] * 0.9998:
...
def get_last_signal_context():
    ...
    return "prev=%s outcome=%s real_move=%+.0f" % (
        last.get("decision", "?"), last.get("outcome", "PENDING"), last.get("real_move", 0))
...
def get_full_payload():
    ...
    return {
        ...
        "price": {"current": price, "chg_15m": chg_15m, "chg_5m": chg_5m,
                  "momentum_3": momentum_3, "micro_mom": micro_mom,
        ...
        "liquidity": {"sweep_15m": sweep_15m, "sweep_5m": sweep_5m, "sweep_1m": sweep_1m,
                      "stops_above": sa, "stops_below": sb,
                      "dist_above": dist_above, "dist_below": dist_below,
                      "fvg5_above": fvg5_a, "fvg5_below": fvg5_b,
                      "fvg1_above": fvg1_a, "fvg1_below": fvg1_b,
                      "bos_choch_5m": bc5m},
        ...
        "positioning": {"funding_rate": fund["rate"], "funding_sent": fund["sentiment"],
                        ...
                        "lsr_bias": lsr["bias"], "lsr_ratio": lsr["ratio"],
                        "crowd_long": lsr["long_pct"]},
        "context": {"volatility": vol_cond, "vol_score": vol_score,
                    "session": session, "session_boost": sess_boost,
                    "market_condition": mkt_cond, "last_signal": last_sig_ctx},
    }
...
def get_balance():
    try:
        ...
        if key.lower().startswith("0x"):
            key = key[2:]
        if len(key) != 64:
            return None, "Ключ має бути 64 символи. Зараз: %d" % len(key)
        int(key, 16)
        account = Account.from_key(key)
        addr    = account.address
        usdc    = "0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174"
        call_d  = "0x70a08231000000000000000000000000" + addr[2:]
        payload = {"jsonrpc":"2.0","method":"eth_call",
                   "params":[{"to":usdc,"data":call_d},"latest"],"id":1}
        rpcs = [ALCHEMY_RPC_URL,"https://polygon.drpc.org",
        ...
        for rpc in rpcs:
            ...
            try:
                ...
                if d and "result" in d and d["result"] not in ("0x","0x0",None,""):
                    return round(int(d["result"],16)/1e6, 2), addr
            except Exception:
                continue
        return None, "Всі RPC недоступні."
    except ImportError:
        return None, "pip install eth-account"
    except Exception as e:
        return None, str(e)

def find_market():
    try:
        for q in ["BTC Up or Down 15","Bitcoin Up or Down 15","Bitcoin 15 minutes","BTC 15 min"]:
            markets = safe_get("https://gamma-api.polymarket.com/markets",
                               {"q": q, "active": "true", "limit": 20})
            if not markets:
                continue
            for m in markets:
                ...
                if ("btc" in t or "bitcoin" in t) and ("15" in t or "up or down" in t):
                    ...
                    if mid:
                        return mid, m.get("question","BTC 15min")
        return None, None
    except Exception as e:
        logger.error("Market: %s", e)
        return None, None
...
def place_bet(market_id, outcome, amount):
    try:
        ...
        if not token:
            return {"success": False, "error": "Outcome %s не знайдено" % outcome}
        price = float(token.get("price",0.5))
        order = client.create_and_post_order(OrderArgs(
            token_id=token["token_id"], price=price, size=amount,
...
============================
TIMEFRAME LOGIC (critical change)
============================
We now use 15m + 5m + 1m instead of 1H + 15m:
- 15m = context (what market is doing over last hour)
- 5m  = tactics (entry confirmation, FVG, BOS/CHoCH)
- 1m  = execution (micro momentum, very recent movement)
This is MORE accurate for 15-minute predictions than using 1H.
...
Manipulation = liquidity sweep (stops taken above or below range)
THIS IS THE KEY: Sweep HIGH does NOT always mean DOWN

SWEEP LOW:
  Price dips below range low, takes stops, closes BACK ABOVE = smart money BOUGHT
  → Strong UP signal (+3)

SWEEP HIGH:
  Case A: price spikes above range, closes BELOW sweep level = smart money SOLD
  → DOWN signal (+3) [this is confirmed reversal]
  Case B: price spikes above range, HOLDS ABOVE sweep level = just stop hunt, trend continues
  → UP or NEUTRAL signal (NOT DOWN)

ALWAYS check AMD.reason field — it tells you which case applies.
If AMD says "HOLDS_ABOVE" or "bull continuation" → lean UP, not DOWN.

Distribution = strong directional move already happening
FVG forms during distribution — acts as a magnet
Price returns to FVG to fill the imbalance

============================
RANGING MARKET RULES
============================
When 15m structure = RANGING:
  dist_below < 0.1% = price at range bottom = bounce UP likely
  dist_above < 0.1% = price at range top = rejection DOWN likely
  dist_above < 0.05% AND sweep_15m = HIGH AND close < sweep_level = STRONG DOWN
  dist_below < dist_above AND < 0.3% = lean UP
  dist_above < dist_below AND < 0.3% = lean DOWN
  both distances > 0.5% = use momentum + orderbook

CHOP_ZONE trap = price took BOTH sides = dangerous = reduce strength to LOW
...
+3 AMD MANIPULATION_DONE direction=UP (confirmed)
+2 dist_below < 0.1% (range bottom bounce)
+2 CHoCH UP on 5m
+2 BOS UP on 5m
+2 SHORT_SQUEEZE_FUEL liquidations
+2 SWEEP_TRAP_LOW reversal UP
+1 FVG_5m below dist < 0.15% (magnet UP)
+1 FVG_1m below dist < 0.10% (strong magnet UP)
+1 dist_below < dist_above AND dist_below < 0.3%
+1 BID_HEAVY orderbook (>+20%)
+1 CROWD_SHORT + SHORTS_TRAPPED funding
+1 micro_mom_1m > +0.03% (1m momentum up)
+1 OI rising AND price rising
+1 LONDON session
+1 structure_5m = BULLISH

-3 AMD MANIPULATION_DONE direction=DOWN (confirmed close_below sweep)
-2 dist_above < 0.1% (range top rejection)
-2 CHoCH DOWN on 5m
-2 BOS DOWN on 5m
-2 LONG_CASCADE_FUEL liquidations
-2 SWEEP_TRAP_HIGH reversal DOWN (only if close_below confirmed)
-1 FVG_5m above dist < 0.15% (magnet DOWN)
-1 FVG_1m above dist < 0.10% (strong magnet DOWN)
-1 dist_above < dist_below AND dist_above < 0.3%
-1 ASK_HEAVY orderbook
-1 CROWD_LONG + LONGS_TRAPPED funding
-1 micro_mom_1m < -0.03%
-1 OI rising AND price falling
-1 structure_5m = BEARISH

PENALTIES:
-1 CHOP_ZONE detected (reduce confidence, market is chaotic)
-1 DEAD_HOURS session (low liquidity, less reliable)
-1 exhaustion = true (late in move, reversal risk)

SPECIAL RULE:
If dist_above < 0.05% AND sweep_15m = HIGH AND price closed below sweep = -3 (very strong resistance confirmed)
...
|score| 3-4 = MEDIUM
|score| 1-2 = LOW
score = 0 = use micro_mom_1m direction, LOW

ALWAYS output UP or DOWN. Never refuse. Never NO_TRADE.
...
============================
{
  ...
  "amd_used": true or false,
  "key_signal": "one sentence — the single most important signal driving this decision",
  "logic": "2-3 sentences Ukrainian — full reasoning",
  "reasons": ["signal 1", "signal 2", "signal 3"],
  "risk_note": "any red flags or conflicting signals"
}"""
...
# ============================================================
# HISTORY + SIGNALS DUMP (новий — повний дамп для аналізу)
# ============================================================

def save_signal_dump(payload, result, outcome_placeholder="PENDING"):
    """
    Зберігає ПОВНИЙ запис кожного сигналу: всі дані що зчитав бот + рішення AI.
    Після 15 хвилин outcome оновлюється через check_signals.
    Скидай цей файл мені для аналізу.
    """
    try:
        dump = []
        if os.path.exists(SIGNALS_DUMP):
            with open(SIGNALS_DUMP) as f:
                dump = json.load(f)
        liq = payload["liquidity"]
        pos = payload["positioning"]
        pr  = payload["price"]
        ...
        record = {
            ...
            "amd_used":         result.get("amd_used"),
            "market_condition": result.get("market_condition"),
            # --- PRICE ---
            "price":       pr["current"],
            "chg_15m":     pr["chg_15m"],
            "chg_5m":      pr["chg_5m"],
            "momentum_3":  pr["momentum_3"],
            "micro_mom":   pr["micro_mom"],
            "mark":        pr["mark"],
            "basis":       pr["basis"],
            # --- STRUCTURE ---
            "st15m": st["15m"],
            ...
            # --- CONTEXT ---
            "session":      ctx["session"],
            "sess_boost":   ctx["session_boost"],
            "volatility":   ctx["volatility"],
            "vol_score":    ctx["vol_score"],
            "mkt_cond_ctx": ctx["market_condition"],
            "last_signal":  ctx["last_signal"],
            # --- AMD ---
            "amd_phase":       amd.get("phase"),
            "amd_direction":   amd.get("direction"),
            "amd_confidence":  amd.get("confidence"),
            "amd_reason":      amd.get("reason"),
            "amd_sweep_level": amd.get("sweep_level"),
            # --- SWEEPS ---
            "sweep15m_type":  sw15.get("type"),
            "sweep15m_level": sw15.get("level"),
            "sweep15m_ago":   sw15.get("ago"),
            "sweep5m_type":   sw5.get("type"),
            "sweep5m_level":  sw5.get("level"),
            "sweep5m_ago":    sw5.get("ago"),
            "sweep1m_type":   sw1.get("type"),
            "sweep1m_level":  sw1.get("level"),
            "sweep1m_ago":    sw1.get("ago"),
            # --- STOPS / DIST ---
            "stops_above_price": sa.get("price"),
            "stops_above_type":  sa.get("type"),
            "stops_below_price": sb_.get("price"),
            "stops_below_type":  sb_.get("type"),
            "dist_above": liq.get("dist_above"),
            "dist_below": liq.get("dist_below"),
            # --- FVG ---
            "fvg5_above_dist": f5a["dist"] if f5a else None,
            "fvg5_above_size": f5a["size"] if f5a else None,
            "fvg5_below_dist": f5b["dist"] if f5b else None,
            "fvg5_below_size": f5b["size"] if f5b else None,
            "fvg1_above_dist": f1a["dist"] if f1a else None,
            "fvg1_below_dist": f1b["dist"] if f1b else None,
            # --- BOS ---
            "bos5m_type":  bc5["type"]  if bc5 else None,
            "bos5m_dir":   bc5["dir"]   if bc5 else None,
            "bos5m_level": bc5["level"] if bc5 else None,
            # --- MANIPULATION ---
            "trap_type":       manip["trap_type"],
            "reversal_signal": manip["reversal_signal"],
            # --- POSITIONING ---
            "funding_rate": pos["funding_rate"],
            "funding_sent": pos["funding_sent"],
            "liq_longs":    pos["liq_longs"],
            "liq_shorts":   pos["liq_shorts"],
            "liq_signal":   pos["liq_signal"],
            "exhaustion":   pos["exhaustion"],
            "liq_total":    pos["liq_total"],
            "oi":           pos["oi"],
            "oi_change":    pos["oi_change"],
            "ob_bias":      pos["ob_bias"],
            "ob_imbalance": pos["ob_imbalance"],
            "lsr_bias":     pos["lsr_bias"],
            "lsr_ratio":    pos["lsr_ratio"],
            "crowd_long":   pos["crowd_long"],
        }
        dump.append(record)
        dump = dump[-1000:]   # зберігаємо до 1000 записів
        with open(SIGNALS_DUMP, "w") as f:
            json.dump(dump, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.warning("Dump save: %s", e)

def update_dump_outcome(ts_unix_val, outcome, exit_price, real_move):
    """Оновлює outcome в signals_dump після того як результат відомий"""
    try:
        ...
        with open(SIGNALS_DUMP) as f:
            dump = json.load(f)
        for rec in dump:
            if rec.get("ts_unix") == ts_unix_val and rec.get("outcome") == "PENDING":
                rec["outcome"]    = outcome
                rec["exit_price"] = exit_price
                rec["real_move"]  = real_move
                break
        with open(SIGNALS_DUMP, "w") as f:
            json.dump(dump, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.warning("Dump update: %s", e)
    ...
    for sig in signal_history:
        ...
        if now - sig.get("ts_unix",0) >= 900:
            ...
            if cur:
                ...
                sig["outcome"]    = outcome
                sig["exit_price"] = cur
                sig["real_move"]  = real_move
                changed = True
                if outcome == "LOSS":
                    save_error(sig)
                # Оновлюємо dump
                update_dump_outcome(sig.get("ts_unix"), outcome, cur, real_move)
    if changed:
        save_history()
...
async def cmd_resetstats(u,c):
    ...
    # SIGNALS_DUMP не чистимо — він для аналізу
    await u.message.reply_text("Статистика очищена.\n(signals_dump.json збережено для аналізу)")
...
async def run_cycle(app):
    ...
    # Зберігаємо повний дамп для подальшого аналізу
    save_signal_dump(payload, result)
