...
def init_poly_client(s):
    """Перевіряємо підключення без py-clob-client"""
    if not s.wallet_ok:
...
def find_btc_market(verbose=False):
    """
    Знаходить "Bitcoin Up or Down - 15 min" через slug btc-updown-15m-{timestamp}.
    Токени беремо через CLOB API по condition_id.
    """
    import time as _t
    import math
    ...
    def get_tokens_from_clob(condition_id):
        """Отримує token_id YES/NO через CLOB по condition_id"""
        try:
            r = requests.get(
                "https://clob.polymarket.com/markets/%s" % condition_id,
                timeout=15)
            if r.status_code != 200:
                print("[CLOB] HTTP %d for %s" % (r.status_code, condition_id[:20]))
                return None, None, 0.5, 0.5
            m = r.json()
            tokens = m.get("tokens", [])
            print("[CLOB] Got %d tokens for %s" % (len(tokens), condition_id[:20]))
            ty_id = tn_id = ""
            ty_pr = tn_pr = 0.5
            for t in tokens:
            ...
            print("[CLOB] Token fetch error: %s" % e)
            return None, None, 0.5, 0.5
    ...
    def try_slug(slug, now_ts):
        try:
            ...
            for m in markets:
                ...
                # Токени через CLOB (бо в Gamma вони порожні)
                ty_id, tn_id, ty_pr, tn_pr = get_tokens_from_clob(cid)

                if not ty_id or not tn_id:
                    print("[Market] No tokens for cid=%s" % cid[:20])
                    continue

                print("[MARKET] FOUND: %s" % q[:60])
                print("[MARKET] condition_id=%s" % cid)
                print("[TOKENS] YES=%s price=%.4f" % (ty_id[:25], ty_pr))
                print("[TOKENS] NO =%s price=%.4f" % (tn_id[:25], tn_pr))
                print("[MARKET] closes in %.0f sec" % diff)
            ...
            print("[Market] try_slug error: %s" % e)
        return None

    def scan():
        now_ts = _t.time()
        # Timestamp поточного раунду (кратний 900)
        current = int(now_ts // ROUND_SEC) * ROUND_SEC
        # Пробуємо поточний, наступний і попередній раунд
        timestamps = [current, current + ROUND_SEC, current - ROUND_SEC]
        print("[Market] now=%.0f current_round=%d" % (now_ts, current))

        for ts in timestamps:
            slug   = "%s%d" % (SLUG_PREFIX, ts)
            print("[Market] Trying slug: %s" % slug)
            result = try_slug(slug, now_ts)
            if result:
                return result

        print("[Market] All slugs failed")
        return None
...
def place_bet(s, direction: str, amount: float) -> dict:
    ...
    Використовує py-clob-client з signature_type=1 (EOA/MetaMask гаманець).
    ВАЖЛИВО: гаманець має бути зареєстрований на polymarket.com
    """
    if not s.wallet_ok:
    ...
    try:
        ...
        # signature_type=1 = EOA гаманець (MetaMask)
        # Спочатку ініціалізуємо без API ключів щоб отримати їх
        client = ClobClient(
            host="https://clob.polymarket.com",
            key=key,
            chain_id=POLYGON,
            signature_type=0,  # 0 = Magic.Link (email), 1 = MetaMask
        )

        # Отримуємо або створюємо API credentials
        try:
            creds = client.derive_api_key()
            print("[BET] derive_api_key OK: %s..." % str(creds)[:50])
        except Exception as e1:
            print("[BET] derive_api_key failed: %s" % e1)
            try:
                creds = client.create_api_key()
                print("[BET] create_api_key OK: %s..." % str(creds)[:50])
            except Exception as e2:
                print("[BET] create_api_key failed: %s" % e2)
                return {"success": False,
                        "error": "Cannot get API key. Register wallet on polymarket.com first! Error: %s" % str(e2)[:100]}

        # Встановлюємо credentials і реініціалізуємо з ними
        ...
        print("[BET] Order OK: %s" % str(resp)[:100])
        ...
        return {"success": False,
                "error": "py-clob-client not installed. Check requirements.txt and redeploy."}
    except Exception as e:
        err = str(e)
        print("[BET] FAIL: %s" % err)
        if "not registered" in err.lower() or "user not found" in err.lower():
            return {"success": False,
                    "error": "Wallet not registered on Polymarket. Go to polymarket.com and sign up first!"}
        ...
        if "401" in err or "invalid l1" in err.lower():
            return {"success": False,
                    "error": "Auth error 401. Register wallet on polymarket.com first, then retry."}
        if "nonce" in err.lower():
            return {"success": False, "error": "Nonce error, retry in 10s"}
...
async def cmd_rawmarket(u, c):
    """Діагностика: перевіряє slug + CLOB токени"""
    import time as _t
    now    = _t.time()
    ROUND  = 900
    ...
    for ts in [current - ROUND, current, current + ROUND]:
        ...
        try:
            ...
            for m in mkts:
                ...
                try:
                    rc = requests.get(
                        "https://clob.polymarket.com/markets/%s" % cid,
                        timeout=15)
                    if rc.status_code == 200:
                        ...
                        for t in tokens:
                            ...
                            pr  = t.get("price","?")
                            lines.append("  %s: price=%s tid=%s" % (oc, pr, str(tid)[:25]))
                    else:
                        lines.append("CLOB HTTP %d" % rc.status_code)
                except Exception as e:
                    lines.append("CLOB error: %s" % str(e)[:60])
            lines.append("")
        except Exception as e:
            lines.append("error %s: %s" % (slug, str(e)[:60]))
