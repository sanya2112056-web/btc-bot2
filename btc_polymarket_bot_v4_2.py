...
Все інше юзер вводить в Telegram.

Magic.Link авторизація:
- signature_type=1 (POLY_PROXY)
- Приватний ключ → create_or_derive_api_creds() → L2 credentials
- funder = адреса що тримає USDC (з polymarket.com → Deposit)
"""
import asyncio, logging, json, time, datetime, os, io, requests
...
class Session:
    def __init__(self, uid):
        self.uid      = uid
        self.key      = ""     # приватний ключ Magic.Link
        self.funder   = ""     # адреса з polymarket.com → Deposit
        self.address  = None
        self.ok       = False
        self.auto     = False
        self.signals  = []
        self.trades   = []
        self.state    = None   # "key" / "funder"
        self.tmp_key  = None
...
# ─────────────────────────────────────────────
# POLYMARKET CLIENT
# Правильна ініціалізація для Magic.Link:
# signature_type=1 (POLY_PROXY) + funder адреса
# ─────────────────────────────────────────────
def make_clob_client(s):
...
def get_balance(s):
    ...
    try:
        client = make_clob_client(s)
        # create_or_derive_api_creds() — правильний метод для Magic.Link
        creds = client.create_or_derive_api_creds()
        client.set_api_creds(creds)
        info = client.get_balance_allowance(params={"asset_type": "USDC"})
        print("[Balance] відповідь: %s" % str(info)[:150])
        raw = float(info.get("balance") or 0)
        # Polymarket повертає в мікро-USDC (6 decimals)
        bal = raw / 1e6 if raw > 1000 else raw
        bal = round(bal, 2)
        print("[Balance] $%.2f" % bal)
        return bal, s.funder
    except ImportError:
        return 0.0, "py-clob-client не встановлено"
    except Exception as e:
        print("[Balance] помилка: %s" % e)
...
def find_market():
    """
    Знаходить поточний раунд через slug: btc-updown-15m-{timestamp}
    ...
    def try_slug(slug, now):
        try:
            ...
            for m in ev.get("markets", []):
                ...
                print("[Market] YES=%s" % yi[:30])
                print("[Market] NO =%s" % ni[:30])
    ...
    for attempt in range(1, 8):
        print("[Market] Спроба %d/7 | %s UTC" % (
            attempt, datetime.datetime.now(datetime.timezone.utc).strftime("%H:%M:%S")))
...
def place_bet(s, direction: str, amount: float) -> dict:
    """
    Magic.Link торгівля:
    1. make_clob_client() з signature_type=1 + funder
    2. create_or_derive_api_creds() → L2 credentials
    3. create_order() + post_order(GTC)
    """
    ...
    try:
        ...
        return {"ok":True,"resp":resp,"price":price,
                "pot":round(size-amount,2),"mkt":mkt["q"][:60]}

    except ImportError:
        return {"ok":False,"err":"py-clob-client не встановлено. Перевір requirements.txt"}
    except Exception as e:
        err = str(e)
        ...
        if "insufficient" in err.lower():
            return {"ok":False,"err":"Недостатньо коштів"}
...
def lsr():
    try:
        ...
        lat = d[-1]
        r   = float(lat.get("longShortRatio", 1.0))
        lp  = float(lat.get("longAccount", 0.5)) * 100
...
def sweep(c):
    ...
    for i, x in enumerate(reversed(c[-5:])):
        for s in reversed(sh):
            if x["h"]>s["p"] and x["c"]<s["p"]:
                return {"type":"HIGH","level":s["p"],"ago":i+1}
        for s in reversed(sl):
            if x["l"]<s["p"] and x["c"]>s["p"]:
                return {"type":"LOW","level":s["p"],"ago":i+1}
...
def detect_amd(c15, c5, price):
    ...
    if manip and acc:
        if sw["type"]=="LOW":
            return {"phase":"MANIPULATION_DONE","dir":"UP","conf":3 if mc=="UP" else 2,
                    "reason":"ACCUM+SWEEP_LOW UP"}
        lc = c15[-1]["c"]
        if lc < sw["level"]*0.9998:
            return {"phase":"MANIPULATION_DONE","dir":"DOWN","conf":3 if mc=="DOWN" else 2,
                    "reason":"SWEEP_HIGH+CLOSE_BELOW DOWN"}
    ...
    if abs(lm) > 0.15:
        return {"phase":"DISTRIBUTION","dir":"UP" if lm>0 else "DOWN","conf":1,"reason":"active move"}
...
def vol_class(c15):
    ...
    return ("LOW_VOL" if avg<0.08 else "EXPANSION" if rec>pri*1.5 else
...
def stops(c, price):
    sh, sl = swings(c)
    above = [{"p":s["p"],"type":"swing_high"} for s in sh if s["p"]>price]
    below = [{"p":s["p"],"type":"swing_low"}  for s in sl if s["p"]<price]
...
def build_payload(s):
    ...
    f1a, f1b = fvg(c1[-20:], px) if len(c1)>=5 else (None, None)
    ...
    vc, vs = vol_class(c15); sess, sb2 = session(); reg = mkt_regime(c15)
    ...
    return {
        ...
        "ctx":     {"vol":vc,"vs":vs,"sess":sess,"sb2":sb2,"reg":reg,"last":last_s},
    }
...
Analyze fresh every time. Do NOT repeat previous signal direction without new evidence.
...
STRENGTH: >=5=HIGH | 3-4=MEDIUM | 1-2=LOW | ALWAYS output UP or DOWN
OUTPUT JSON only: {"decision":"UP or DOWN","strength":"HIGH or MEDIUM or LOW",
"confidence_score":<int>,"market_condition":"TRENDING or RANGING or CHOPPY",
"key_signal":"one sentence","logic":"2-3 sentences Ukrainian",
"reasons":["r1","r2","r3"],"risk_note":"risk or NONE"}"""
...
def get_news():
    try:
        ...
        if d and "Data" in d:
            bkw = ["bull","surge","rally","etf"]
            skw = ["bear","drop","crash","dump","ban"]
            lines = []
            for item in d["Data"][:5]:
                t  = item.get("title","").lower()
                p_ = sum(1 for k in bkw if k in t)
                n  = sum(1 for k in skw if k in t)
                sent = "BULLISH" if p_>n else "BEARISH" if n>p_ else "NEUTRAL"
...
async def cmd_autoon(u, c):
    s = sess(u.effective_user.id)
    if not s.ok:
        await u.message.reply_text("Спочатку підключи гаманець."); return
    s.auto = True
...
async def on_callback(u, c):
    ...
    if q.data == "wallet":
        ...
        await q.message.reply_text(
            ...
            "Де знайти:\n"
            "polymarket.com → Profile (праворуч вгорі)\n"
            "→ Export Private Key\n\n"
        ...
        if not s.ok:
            await q.message.reply_text("Спочатку підключи гаманець."); return
        if s.auto:
            s.auto = False
        ...
        if bal is not None:
            ...
            await q.message.reply_text("Не вдалось отримати баланс. Перевір логи Railway.")
        ...
        if m:
            ...
            await q.message.reply_text("Маркет не знайдено. Перевір логи Railway.")
        ...
        try:
            ...
            await q.message.reply_text("Помилка читання файлу.")
...
async def on_message(u, c):
    ...
    if s.state == "key":
        ...
        await u.message.reply_text(
            ...
            "Де знайти:\n"
            "polymarket.com → Deposit → скопіюй адресу\n\n"
            "Введи адресу (починається з 0x, 42 символи):")
        return

    # Крок 2: funder адреса
    if s.state == "funder":
        addr = txt.strip()
        if not addr.lower().startswith("0x") or len(addr) != 42:
            ...
            return
        s.state = None
        key = s.tmp_key or ""; s.tmp_key = None
        ...
        try:
            ...
            s.address = s.key[:12] + "..."
        print("[Wallet] key=%s funder=%s" % (s.address, addr))
        bal, _ = get_balance(s); bet = s.bet_size(bal)
        await u.message.reply_text(
            "Гаманець підключено!\n\n"
            "Підписувач: %s\n"
            ...
            "Натисни «Авто: УВІМК»" % (
                s.address, addr,
                ("$%.2f USDC" % bal) if bal is not None else "перевіряємо...",
                bet),
            reply_markup=kb(s))
        return
        ...
        try:
            ...
            ikb = InlineKeyboardMarkup([[
                InlineKeyboardButton(
                    "Підтвердити $%.2f на %s" % (amount, direction),
...
async def cycle(app, s):
    ...
    sig = {
        ...
        "sweep_type":sw15.get("type","NONE"),"amd_phase":ad.get("phase","NONE"),
        "trap":mn["trap"],
    }
    ...
    main_txt = (
        ...
        p["price"]["cur"], mkt_cond, p["ctx"]["sess"],
...
async def scheduler(app):
    while True:
        ...
        await asyncio.sleep(5)  # маркет з'являється через ~5 сек після старту
        if _sessions:
...
def main():
    ...
    async def startup(app):
        ...
        log.info("Бот запущено. Magic.Link. signature_type=1. create_or_derive_api_creds.")
