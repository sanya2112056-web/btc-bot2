...
Railway: тільки TELEGRAM_BOT_TOKEN + OPENAI_API_KEY + POLY_API_KEY + POLY_SECRET + POLY_PASSPHRASE
Magic.Link акаунт | Builder Keys | 10% від балансу
"""
...
# Railway Variables (тільки ці 5 змінних)
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
OPENAI_API_KEY     = os.getenv("OPENAI_API_KEY", "")
# Builder Keys з polymarket.com/settings?tab=builder
...
class Session:
    def __init__(self, uid):
        ...
        self.funder    = ""      # Polymarket proxy адреса (з Deposit)
        self.address   = None
        self.ok        = False
        self.auto      = False
        self.signals   = []
...
# ─────────────────────────────────────────────
# СТАВКА через Builder Keys + py-clob-client
# ─────────────────────────────────────────────
def place_bet(s, direction: str, amount: float) -> dict:
    ...
    if not POLY_API_KEY: return {"ok":False,"err":"POLY_API_KEY не встановлено в Railway"}
    ...
    try:
        ...
        print("[Bet] py-clob err: %s" % str(e)[:150])
    ...
    try:
        ...
        body = _j.dumps({
            "order":{"salt":str(int(time.time()*1000)),"maker":s.funder,"signer":signer,
            ...
            "orderType":"GTC","owner":s.funder,
        }, separators=(",",":"))
        asig = base64.b64encode(
            _h.new(POLY_SECRET.encode(), (order_ts+"POST"+"/order"+body).encode(), hashlib.sha256).digest()
        ).decode()
        ro = requests.post("https://clob.polymarket.com/order", data=body,
            headers={"POLY-API-KEY":POLY_API_KEY,"POLY-SIGNATURE":asig,
                     "POLY-TIMESTAMP":order_ts,"POLY-PASSPHRASE":POLY_PASSPHRASE,
                     "Content-Type":"application/json"}, timeout=20)
        print("[Bet] HTTP %d: %s" % (ro.status_code, ro.text[:150]))
        if ro.status_code in (200,201):
            return {"ok":True,"resp":ro.json(),"price":price,"pot":round(size-amount,2),"mkt":mkt["q"][:60]}
        return {"ok":False,"err":"HTTP%d: %s" % (ro.status_code, ro.text[:150])}
    except Exception as e:
...
Analyze fresh every time. Do NOT repeat previous signal.
...
def analyze_with_ai(p, s):
    try:
        ...
        sa=liq.get("sa") or {}; sb_=liq.get("sb") or {}
...
WELCOME=(
    ...
    "   POLY_SECRET\n"
    "   POLY_PASSPHRASE\n"
    "   (з polymarket.com/settings?tab=builder)\n\n"
    "2. Натисни «Підключити гаманець»\n"
...
async def on_callback(u,c):
    ...
    if q.data=="wallet":
        ...
        if bal is not None:
            ...
            await q.message.reply_text("Не вдалось отримати баланс. Перевір POLY_API_KEY в Railway.")
...
def main():
    ...
    async def startup(app):
        ...
        log.info("Бот запущено. Builder Keys. signature_type=1. 10%% bet.")
